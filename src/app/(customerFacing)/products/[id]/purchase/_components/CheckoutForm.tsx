...
function Form({priceInCents, productId}: {priceInCents: number, productId: string}) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if(orderExists) {
        setErrorMessage('You have already pusrchased this product')
        setIsLoading(false)
        return
    }
  ...
  return <form onSubmit={handleSubmit} >
    <Card>
        ...
        <CardFooter>
            <Button className="w-full" size="lg" disabled={stripe == null || elements == null || loading}>{loading ? 'Purchasing' : `Purhase - ${formatCurrency(priceInCents / 100)}`}</Button>
        </CardFooter>
    </Card>    
  </form>;
}
